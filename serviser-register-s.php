		...
		<div class="container">
			...
			<div class="content">
				<?php
					...
					$radniSat = trim($_POST['radniSat']);
					$radniSat = strip_tags($radniSat);
					$radniSat = htmlspecialchars($radniSat);
					...
					if (empty($radniSat)) {
						$error = true;
						$rsError = "Cena radnog sata nije uneta.";
					} else if (strlen($radniSat) > 5) {
						$error = true;
						$rsError = "Preskup serviser, takve ne primamo.";
					} else if (!preg_match("/^[0-9]*$/",$radniSat)) {
						$error = true;
						$rsError = "Cena radnog sata sme sadrzati samo brojeve.";
					}
					...
					if($error==true) {
						echo "<div class='greska'><p>Greska:<br>$nameError $prezError $telError $emailError $rsError $userError $passError</p><br/>";
						echo "<a href='serviser-register.php'>Nazad</a></div>";
						exit;
					}
					
					echo "<div class='uspesno'><p>Uspesno ste registrovali novog servisera.</p><br/>";
